...
public class DynamicStringInverterTests {
    ...
    Stream<DynamicTest> basicInversion1() {
        ...
        return testVersions("Basic inversion (should succeed)", invert -> isEqual(invert.invert(in), out));
    }
    ...
    @TestFactory
    Stream<DynamicTest> disallowedCharacter() {
    ...
    Stream<DynamicTest> allowedCharacters() {
        ...
        return testVersions("Allowed characters (should succeed)", inverter -> {
            ...
            assertSame(inverter.invert(upcase), lowcase);
            return "success";
        });
    }
    ...
    Stream<DynamicTest> lengthNoGreaterThan30(){
        ...
        return testVersions("Length must be less than 31 (throw exceptions)", inverter -> inverter.invert(str));
    }
    ...
    Stream<DynamicTest> lengthLEssThan31() {
        ...
        return testVersions("Length must be less than 31 (should succeed)", inverter -> inverter.invert(str));
    }
    ...
    Stream<DynamicTest> testVersions(String id, Function<StringInverter, String> test) {
        ...
        return DynamicTest.stream(
                versions.iterator(), inverter -> inverter.getClass().getSimpleName(), inverter -> {
